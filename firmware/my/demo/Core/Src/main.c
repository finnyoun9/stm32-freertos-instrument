...
int main(void)
{
  ...
  vLCDTaskInit();
  vDMMInit();
  WaveformCapture_Init();
  WaveformGenerator_Init(); // 对波形发生器进行初始化
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, LED1_Pin|Buzzer_Pin|LCD_RS_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LED1_Pin Buzzer_Pin LCD_RS_Pin */
  GPIO_InitStruct.Pin = LED1_Pin|Buzzer_Pin|LCD_RS_Pin;
